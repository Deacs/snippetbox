...
import (
	...
	"chilliweb.com/snippetbox/pkg/models/mock"
...
func newTestApplication(t *testing.T) *application {
	...
	session := sessions.New([]byte("j@883r_w0c|<%-@_pO3m4alL8|`|`rQD"))
...
func newTestServer(t *testing.T, h http.Handler) *testServer {
	...
	// is called after a 3xx response is received by the client, asnd returning
	// the http.ErrUseLastResponse error forces ito to immediately return the
	// received response.
...
// request to as given url path on the test server, asnd returns the response
