...
func TestShowSnippet(t *testing.T) {
	...
	// Establish a new tert server for running end to end tests.
	ts := newTestServer(t, app.routes())
	defer ts.Close()
	...
	tests := []struct {
		...
		{"Valid ID", "/snippet/1", http.StatusOK, []byte("An old and silent pond...")},
		{"Non-existent ID", "/snippet/2", http.StatusNotFound, nil},
		{"Negative ID", "/snippet/-1", http.StatusNotFound, nil},
		{"Decimal ID", "/snippet/1.23", http.StatusNotFound, nil},
		{"String ID", "/snippet/foo", http.StatusNotFound, nil},
		{"Enpty ID", "/snippet/", http.StatusNotFound, nil},
...
func TestSignupUser(t *testing.T) {
	...
	// Add the table driven sub-tests
	tests := []struct {
		name         string
		userName     string
		userEmail    string
		...
		{"Empty name", "", "bob@example.com", "validPa$$word", csrfToken, http.StatusOK, []byte("This field cannot be left blank")},
		{"Empty email", "Bob", "", "validPa$$word", csrfToken, http.StatusOK, []byte("This field cannot be left blank")},
		{"Empty password", "Bob", "bob@example.com", "", csrfToken, http.StatusOK, []byte("This field cannot be left blank")},
