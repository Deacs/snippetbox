...
func TestSecureHeaders(t *testing.T) {
	...
	// middleware, which write a 200 status code and an "OK" response body.
	next := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte("OK"))
	})

	// Pass the mock HTTP handler to our secureHeaders middleware. Because
	// secureHeaders *returns* a http.Handler we can call its ServerHTTP()
	...
	// Check that the middleware has corectly set the X-XSS-Protection header
	// on the response.
	...
	// Check that he middleware has correctly called the next handler in line
