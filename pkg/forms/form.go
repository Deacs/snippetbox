...
// Use the regexp.MustCompile() function to parse and compile a
// regular expression for sanity checking the format of an email address.
// This returns a *regexp.Regexp object , or panics in the event opf an error.
...
// matches one of a set of specified permitted values. If the check fails
...
// contains a minimum number of charactewrs. If the check fails then add the
// appropriate message to the form errors.
...
// matches the regular expression. If the check fails then add the appropriate
